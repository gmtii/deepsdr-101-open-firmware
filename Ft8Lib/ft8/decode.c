...
#if !defined(__arm__) && defined(FT8_LLR_DEBUG_DUMP)
#include <stdio.h>
#endif

// GD32F450VE PORT (09/2026): direct Cortex-M4F hardware sqrt, no libm -
// see ftx_normalize_logl()'s comment for why. "t" constrains the operand
// to a single-precision FPU register (s0-s31); VSQRT.F32 is a single
// instruction, no approximation involved (unlike fft.c's log2_approx()).
// Falls back to plain sqrtf() on non-ARM builds (the x86 host test
// harness - see test/host/ - which links this same file unmodified to
// validate the decode chain on real WAV captures before ever touching
// hardware).
static inline float gd32_ft8_fast_sqrtf(float x)
{
#if defined(__arm__)
    float result;
...
static void ftx_normalize_logl(float* log174)
{
    ...
    //
    // GD32F450VE PORT (09/2026): sqrtf() replaced with a direct VSQRT.F32
    // hardware instruction. Not a link-time necessity - this project's
    // Makefile already links -lm (added 31/07/2026 for demod_am.c's
    // atan2f()), so libm's sqrtf() would have worked here too - this is
    // purely an optimization: one FPU instruction instead of a libm call,
    // on a value computed once per decode attempt. See
    // gd32_ft8_fast_sqrtf() above.
    float norm_factor = gd32_ft8_fast_sqrtf(24.0f / variance);
    for (int i = 0; i < FTX_LDPC_N; ++i)
    {
...
#if !defined(__arm__) && defined(FT8_LLR_DEBUG_DUMP)
    {
        float mn = log174[0], mx = log174[0], sum = 0.0f, sumabs = 0.0f;
        int i;
        for (i = 0; i < FTX_LDPC_N; i++) {
            if (log174[i] < mn) mn = log174[i];
            if (log174[i] > mx) mx = log174[i];
            sum += log174[i];
            sumabs += (log174[i] < 0 ? -log174[i] : log174[i]);
        }
        printf("LLR DIAG cand freq_off=%d time_off=%d score=%d: min=%.2f max=%.2f mean=%.2f meanabs=%.2f  first8=[",
               cand->freq_offset, cand->time_offset, cand->score, mn, mx, sum/FTX_LDPC_N, sumabs/FTX_LDPC_N);
        for (i = 0; i < 8; i++) printf("%.2f ", log174[i]);
        printf("]\n");
    }
#endif
